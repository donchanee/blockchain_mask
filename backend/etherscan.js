...
exports.getTokenInfofromWallet = ((req, res)=>{
    let url = util.format('https://api-ropsten.etherscan.io/api?module=account&action=tokennfttx&contractaddress=%s&address=%s&page=1&offset=1000&sort=asc&apikey=%s', contractaddress, req.params.address, myApi);
    request(url, function(err, response, body){
        let data = new Object();
        if(!err && response.statusCode == 200){
            data = {
